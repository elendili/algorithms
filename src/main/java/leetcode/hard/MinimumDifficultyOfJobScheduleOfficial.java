...
public class MinimumDifficultyOfJobScheduleOfficial {
    ...
    public int minDifficulty(int[] jobDifficulty, int d) {
        ...
        for (int day = 0; day < d; ++day) {
            ...
            for (int i = day; i < n; i++) {
                // We initialize min_diff_curr_day[integer] as only performing 1 job at the integer-th index.
                // At day 0, the minimum total job difficulty is to complete the 0th job only.
                // Otherwise, we increment min_diff_prev_day[integer - 1] by the integer-th job difficulty
                ...
                while (!stack.isEmpty() && jobDifficulty[stack.peek()] <= jobDifficulty[i]) {
                    // If we include all jobs with index j+1 to integer to the current d,
                    // total job difficulty of the current d will be increased
                    // by the amount of jobDifficulty[integer] - jobDifficulty[j]
                    int j = stack.pop();
